...
{
    Injector::Injector(O2MusicHook::Launcher::Process *process) :
        process(process),
        remoteAddr(0),
        remoteThread(0)
    {
    ...
    HANDLE Injector::Inject(std::string dll_name)
    {
        ...
        {
            std::fprintf(stderr, "O2MUSICHOK: DLL to inject not found (%s).\n", dll_name.c_str());
            return 0;
        }
        ...
        if (!remoteAddr)
        {
            std::fprintf(stderr, "O2MUSICHOK: Failed to allocate DLL memory (%d).\n", GetLastError());
            return 0;
        }
        ...
        if (!success) 
        {
            std::fprintf(stderr, "O2MUSICHOK: Failed to write DLL into memory (%d).\n", GetLastError());
            Close();
        ...
        if (!remoteThread) 
        {
            std::fprintf(stderr, "O2MUSICHOK: Failed to CreateRemoteThread (%d).\n", GetLastError());
            Close();
        ...
        if (WaitForSingleObject(remoteThread, INFINITE) != WAIT_OBJECT_0)
            std::fprintf(stderr, "O2MUSICHOK: Failed to wait object (%d).\n", GetLastError());

        DWORD result;
        if (!GetExitCodeThread(remoteThread, &result))
            std::fprintf(stderr, "O2MUSICHOK: Failed to wait exit thread (%d).\n", GetLastError());

        if (result == 0)
            std::fprintf(stderr, "O2MUSICHOK: Failed to load inside target process (%d).\n", GetLastError());
    ...
    bool Injector::Close()
    {
        ...
        if (remoteAddr) 
        {
            ...
            if (!success)
            {
                std::fprintf(stderr, "O2MUSICHOK: Failed to clear allocated dll memory (%d).\n", GetLastError());
                return false;
            }
        }
